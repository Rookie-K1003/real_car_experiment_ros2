...
public:
    ControlAgent() : Node("control_agent")
    {
        ...
        // ----发布方----
        _control_cmd_publisher = this->create_publisher<control_msgs::msg::ControlReq>(
            "/tracking_control_req",
            10
        );
...
private:
    ...
    // ----发布方----
    // 控制指令发布方
    rclcpp::Publisher<control_msgs::msg::ControlReq>::SharedPtr _control_cmd_publisher;
...
public:
    void control_run_step()
    {
        ...
        // 3.发布控制指令
        control_msgs::msg::ControlReq cmd_msg;
        cmd_msg.brake_req = cmd.brake;
        cmd_msg.steer_req = cmd.steer;
        cmd_msg.throttle_req = cmd.throttle;
...
private:
    ...
    void emergency_stop()
    {
        control_msgs::msg::ControlReq control_msg;
        control_msg.brake_req = 1.0;
        control_msg.throttle_req = 0.0;
        control_msg.steer_req = 0.0;
