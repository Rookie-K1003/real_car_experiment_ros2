...
{   _current_time = this->now().seconds();
    ...
    // -----------------------------------3.路径规划--------------------------------------
    // 3.1获取车道宽度
    double lane_width = 4.0; // 先假定车道宽度为4m
 ...
 * @brief 计算规划起点
 *
 * 根据车辆当前状态，计算规划起点。如果是第一次运行，则直接将车辆当前状态作为规划起点；
 * 如果不是第一次运行，则根据车辆当前状态与上一周期规划轨迹的误差来判断规划起点的位置。
 * 如果误差较小，则在上一周期轨迹上搜索规划起点；如果误差较大，则使用动力学方程外推得到规划起点。
 *
