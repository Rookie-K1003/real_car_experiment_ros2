...
public:
        ...
        : Node("can_decode", node_options)
    {
        ...
        // 读取配置文件
        // 读取配置文件中的参数
        this->declare_parameter("localization.latitude_origin", 31.291673799999998);
        this->declare_parameter("localization.longitude_origin", 121.208718499999989);
        this->declare_parameter("localization.altitude_origin", 18.95);
    ...
    void main_process()
    {
        ...
        while (rclcpp::ok()) {
            ...
            // 打印地图原点的xyz
            cout << "longitude_origin_" << longitude_origin_ << endl;
            cout << "latitude_origin_" << latitude_origin_ << endl;
            cout << "altitude_origin_" << altitude_origin_ << endl;
            // 将lla转map坐标系下的x,y,z
            gps2xy(longitude_origin_, latitude_origin_, altitude_origin_);
...
private:
    ...
    void gps2xy(const double &longitude, const double &latitude, const double &height)
    {
        
        double Re = R / sqrt(1 - e * e * sin(latitude * torad) * sin(latitude * torad));
        ...
        Re = R / sqrt(1 - e * e * sin(latitude_ * torad) * sin(latitude_ * torad));
        double dx_ECEF = (Re + altitude_) * cos(latitude_ * torad) * cos(longitude_ * torad) - x0_ECEF;
        double dy_ECEF = (Re + altitude_) * cos(latitude_ * torad) * sin(longitude_ * torad) - y0_ECEF;
        double dz_ECEF = (Re * (1 - e * e) + altitude_) * sin(latitude_ * torad) - z0_ECEF;

        // ECEF to ENU
        map_x_ = -sin(longitude_ * torad) * dx_ECEF + cos(longitude_ * torad) * dy_ECEF;
        map_y_ = -sin(latitude_ * torad) * cos(longitude_ * torad) * dx_ECEF - sin(latitude_ * torad) * sin(longitude_ * torad) * dy_ECEF + cos(latitude_ * torad) * dz_ECEF;
        map_z_ = cos(latitude_ * torad) * cos(longitude_ * torad) * dx_ECEF + cos(latitude_ * torad) * sin(longitude_ * torad) * dy_ECEF + sin(latitude_ * torad) * dz_ECEF;
    }
    /**
