...
public:
    ...
    void main_process()
    {
        ...
        int32_t ret = canOpen(channel, mode, NTCAN_MAX_TX_QUEUESIZE,
                              NTCAN_MAX_RX_QUEUESIZE, 5, 5, &dev_handler_); //@JackFine 此处打开can驱动
        ...
        */
        ...
        // tj int reclen = 0;
        //  @JackFine 此处为拿取的一包原始can数据，预留的空间，需要根据dbc确定大小
        ...
        while (rclcpp::ok())
        {
            ...
            if (ret2 == NTCAN_SUCCESS && frame_num != 0) // 调用接收函数,得到数据
            {
                ...
                for (int i = 0; i < frame_num; ++i)
                {
                    ...
                    if (recv[i].id == 0x324)
                    {
                        rtkdata.latitude = CAN_decode(recv[i], 0, 32, 1e-7, 0, 0);
                        rtkdata.longitude = CAN_decode(recv[i], 32, 32, 1e-7, 0, 0);
                        // printf("longitude :s %f , latitude : %f \n", rtkdata.longitude, rtkdata.latitude);
                    }

                    if (recv[i].id == 0x321)
                    {
                        rtkdata.angrate_raw_x = CAN_decode(recv[i], 0, 20, 1e-2, 0, 1) * torad;
                        rtkdata.angrate_raw_y = CAN_decode(recv[i], 20, 20, 1e-2, 0, 1) * torad;
                        rtkdata.angrate_raw_z = CAN_decode(recv[i], 40, 20, 1e-2, 0, 1) * torad;
                    }
                    // printf("rtkdata.angrate_raw_x : %f \n", rtkdata.angrate_raw_x);
                    // printf("rtkdata.angrate_raw_y : %f \n", rtkdata.angrate_raw_y);
                    // printf("rtkdata.angrate_raw_z : %f \n", rtkdata.angrate_raw_z);

                    if (recv[i].id == 0x322)
                    {
                        rtkdata.accel_raw_x = CAN_decode(recv[i], 0, 20, 1e-4, 0, 1) * g;
                        rtkdata.accel_raw_y = CAN_decode(recv[i], 20, 20, 1e-4, 0, 1) * g;
                        rtkdata.accel_raw_z = CAN_decode(recv[i], 40, 20, 1e-4, 0, 1) * g;
                    }
                    // printf("rtkdata.accel_raw_x : %f \n", rtkdata.accel_raw_x);
                    // printf("rtkdata.accel_raw_y : %f \n", rtkdata.accel_raw_y);
                    // printf("rtkdata.accel_raw_z : %f \n", rtkdata.accel_raw_z);

                    if (recv[i].id == 0x325)
                    {
                        rtkdata.height = CAN_decode(recv[i], 0, 32, 1e-3, 0, 0);
                        // printf("height : %f \n", rtkdata.height);
                    }

                    if (recv[i].id == 0x32A)
                    {
                        rtkdata.heading = CAN_decode(recv[i], 0, 16, 1e-2, 0, 0);
                        rtkdata.pitch = CAN_decode(recv[i], 16, 16, 1e-2, 0, 1);
                        rtkdata.roll = CAN_decode(recv[i], 32, 16, 1e-2, 0, 1);
                    ...
                    if (recv[i].id == 0x327)
                    {
                        rtkdata.velocity = CAN_decode(recv[i], 45, 16, 1e-2, 0, 0);
                    }
                }
            }
            // @JackFine 判断rtk数据是否更新
...
private:
    ...
    double CAN_decode(CMSG raw_data, int lsb, int length, double ratio, double bias, int mode = 0)
    {
        ...
        // std::cout << "lsb_byte = " << lsb_byte << "\n"
        //           << "lsb_bit  = " << lsb_bit << "\n"
        //           << "msb_byte = " << msb_byte << "\n"
        //           << "msb_bit  = " << msb_bit << "\n"
        //           << "num_bytes= " << num_byte << std::endl;

        // @JackFine lsb的位置为单条消息的右下角，不代表具体高位或者低位，该点选择非常重要
        // int lsb_byte = lsb / 8;                       // @JackFine 求低8位的总字节位置
        // int lsb_bit = lsb % 8;                        // @JackFine 求低8位的起始位
        // int msb_bit = 7 - (lsb_bit + length - 1) % 8; // @JackFine 求高8位起始
        // int num_byte;
        // if ((lsb_bit + length - 1) / 8 == 0)
        // num_byte = 1;
        // else
        // num_byte = 1 + 1 + (length - (8 - lsb_bit) - (msb_bit + 1)) / 8; // @JackFine 求字节长度
        // int msb_byte = lsb_byte - num_byte + 1;                          // 求高8位起始字节位置
        ...
        for (int byte = lsb_byte; byte < lsb_byte + num_byte; byte++) // @JackFine 从低到高不断拼接can帧
        {
            ...
            // std::cout << "the bit of data = " << bitset<sizeof(data) * 8>(data) << std::endl;
            // printf("calculate byte %d : %lf \n", byte, data);
        }
        // std::cout << "the FINAL bit of data = " << bitset<sizeof(data) * 8>(data) << std::endl;
        ...
        //         for (int i = lsb_bit; i <= 7; i++)
        //             tmp += round(pow(2, i));
        //         data += (int)((raw_data.data[byte] & tmp) << 24); // @JackFine 提取有效字段
        //         deviation += (24 - 8);
        //     }
        //     else if (byte == lsb_byte - num_byte + 1) // @JackFine 最后一个字节
        //     {
        ...
        //         for (int i = 0; i <= msb_bit; i++)
        //             tmp += round(pow(2, i));
        //         data += (int)((raw_data.data[byte] & tmp)); // @JackFine 提取有效字段
        //     }
        //     // else if (byte == lsb_byte - 1)
        //     else
        //     {
        //         data += (int)((raw_data.data[byte]) << deviation); // @JackFine 读取所有的数据
        //         deviation -= 8;
    ...
    double CAN_decode_old(CMSG raw_data, int lsb, int length, double ratio, double bias, int mode = 0)
    {
        int lsb_byte = lsb / 8;                   // @JackFine 求低8位的总字节位置
        int lsb_bit = lsb % 8;                    // @JackFine 求低8位的起始位
        int msb_bit = (lsb_bit + length - 1) % 8; // @JackFine 求高8位起始
